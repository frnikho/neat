    ...
    .group('/user', (app) =>
        app
            .get(
                '/:userId',
                ({auth, params}) =>
                    response(getUser({auth, userId: params.userId})),
                {
    ...
    .group('/user/:userId/profile-picture', (app) =>
        app
            .post(
                ...
                ({body, auth, params}) =>
                    response(
                        uploadProfilePicture({auth, ...body, userId: params.userId})
                    ),
                {body: 'user.request.profile-picture', detail: {tags: ['User']}}
            )
            .get('/', () => {
            }, {detail: {tags: ['User']}})
            .delete(
                '/',
                ({auth, params}) =>
                    response(deleteProfilePicture({auth, userId: params.userId})),
                {detail: {tags: ['User']}}
            )
    );
