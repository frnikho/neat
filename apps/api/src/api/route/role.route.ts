...
import { RoleRequest, RoleResponse } from "@api/dto/role.dto";
...
export default new Elysia()
    ...
    .group('/role', (app) =>
        app
            .get('/', ({auth, query}) => response(listRole({
                auth,
                pag: extractFromQuery(query)
            })), {query: 'pagination', response: {200: 'role.response.list'}, detail: {tags: ['Role']}})
            .post('/', ({body, auth}) => response(createRole({
                auth,
                body
            })), {
            ...
            .delete('/:id', ({params, auth}) => response(deleteRole({
                auth,
                roleId: params.id
            })), {response: 'role.response.delete', detail: {tags: ['Role']}})
            .get('/:id', ({params, auth}) => response(getRole({
                auth,
                roleId: params.id
            })), {response: 'role.response.get', detail: {tags: ['Role']}})
            .patch('/:id', ({auth, body, params}) => response(updateRole({
                auth,
                body,
                roleId: params.id
            })), {
