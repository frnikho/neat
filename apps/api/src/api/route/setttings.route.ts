...
import { settingsModels } from "@api/dto/settings.dto";
...
export default new Elysia()
    ...
    .group('/settings', (app) =>
        app
            .get('/', ({query, auth}) => response(listSettings({auth, pag: extractFromQuery(query)})), {query: 'pagination', tags: ['Settings']})
            .get('/:key', ({params, auth}) => response(getSettings({auth, key: params.key})), {params: 'settings.key', tags: ['Settings']})
            .put('/:key', ({params, body, auth}) => response(updateSettings({key: params.key, auth, body})), {body: 'settings.update', params: 'settings.key', tags: ['Settings']})
    )