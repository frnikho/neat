...
export default (client: NodePgDatabase): UserRoleInterface => ({
	...
	remove: (userId, roleId) => {
		return op(client.delete(user_role).where(and(eq(user_role.userId, userId), eq(user_role.roleId, roleId)))).map(() => {});
	},

	list: (userId) => {
		return op(
			client.select({ role }).from(user_role).innerJoin(role, eq(user_role.userId, userId)).where(eq(user_role.userId, userId)),
