...
import {
  file,
  mapFileMetadata,
  mapFileMetadataOption,
  mapFilesMetadata,
} from '@infra/schema/file.schema';
...
export const fileMetadataRepo = (
  ...
  create: (data) => {
    return op(
      ...
      .andThen((r) =>
        oneOrThrow(r, new DbException('Failed to create file metadata'))
      )
      .map(mapFileMetadata);
  },
  ...
  findByBucket: (bucket) => {
    return op(client.select().from(file).where(eq(file.bucket, bucket))).map(
      mapFilesMetadata
    );
  },
  ...
  update: (id, data) => {
    return op(
      ...
      .andThen((r) =>
        oneOrThrow(r, new DbException('Failed to update file metadata'))
      )
      .map(mapFileMetadata);
  },
  ...
  delete: (id) => {
    return op(client.delete(file).where(eq(file.id, id)).returning())
      .andThen((r) =>
        oneOrThrow(r, new DbException('Failed to delete file metadata'))
      )
      .map(mapFileMetadata);
  },
  ...
  softDelete: (id, deletedBy) => {
    return op(
      ...
      .andThen((r) =>
        oneOrThrow(r, new DbException('Failed to soft delete file metadata'))
      )
      .map(mapFileMetadata);
  },
});
