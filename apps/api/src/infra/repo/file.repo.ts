...
import {FileMetadataInterface, FileObjectInterface} from "@interface/file.interface";
...
export const fileMetadataRepo = (client: NodePgDatabase): FileMetadataInterface => ({
    create: (data) => {
        return op(client.insert(file).values({
            ...data,
    ...
    update: (id, data) => {
        return op(client.update(file).set({
            ...data,
            updatedAt: new Date(),
            updatedBy: data.updatedBy ?? null,
        }).where(eq(file.id, id)).returning())
    ...
    list: (page = 1, limit = 10) => {
        return op(client.select().from(file).limit(limit).offset((page) * (limit)).where(isNotNull(file.deletedAt)))
            .map(mapFilesMetadata);
    },
...
export const fileObjectRepo = (client: S3Client): FileObjectInterface => ({
    save: (file, key, bucket) => {
        return op(client.write(key, file, {bucket}))
            .andThen(() => ok())
    },

    delete: (key, bucket) => {
        return op(client.delete(key, {bucket}))
            .andThen(() => ok())
    }
})