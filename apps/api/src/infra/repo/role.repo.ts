...
export default (client: NodePgDatabase): RoleInterface => ({
    ...
    return op(
      client
        ...
        .innerJoin(
          role_permission,
          and(
            eq(role_permission.roleId, id),
            eq(role_permission.permissionId, permission.id)
          )
        )
    ...
    return op(
      client
        ...
        .where(
          and(
            eq(role_permission.roleId, id),
            inArray(role_permission.permissionId, permissionIds)
          )
        )
        .returning()
    )
  ...
  deleteManyByName: (names) => {
    return op(
      client.delete(role).where(inArray(role.name, names)).returning()
    ).map(mapRoles);
  },
});
