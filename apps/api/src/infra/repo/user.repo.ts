...
export default (db: NodePgDatabase): UserInterface => ({
  ...
  findUserByIdWithProfilePicture: (id) => {
    return op(
      ...
      .map((result) => {
        ...
        return some([
          mapUserToEntity(result.value.user),
          mapFileMetadataOption(fromNullable(result.value.file)),
        ]);
      });
  },
  ...
  list: (page, limit) => {
    return op(
      ...
      return e.map((row) => {
        return [
          mapUserToEntity(row.user),
          mapFileMetadataOption(fromNullable(row.file)),
        ];
      });
    });
  },
  ...
  deletes: (ids) => {
    return op(db.delete(user).where(inArray(user.id, ids)).returning()).map(
      mapUsersToEntities
    );
  },
  ...
  updateProfilePicture: (id, body) => {
    return op(
      ...
      .andThen((r) =>
        oneOrThrow(r, new DbException('Failed to update profile picture'))
      )
      .map(mapUserToEntity);
  },
  ...
  deleteProfilePicture: (id, deletedBy) => {
    return op(
      ...
      .andThen((r) =>
        oneOrThrow(r, new DbException('Failed to delete profile picture'))
      )
      .map(mapUserToEntity);
  },
});
