...
export const user = pgTable('user', {
  ...
  profilePictureFile: varchar('profile_picture_file').references(
    (): AnyPgColumn => file.id,
    { onDelete: 'set null' }
  ),
  profilePictureUpdatedAt: timestamp('profile_picture_updated_at'),
  profilePictureUpdatedBy: varchar('profile_picture_updated_by').references(
    (): AnyPgColumn => user.id,
    { onDelete: 'set null' }
  ),
...
export const mapUserOption = (row: Option<User>) =>
  mapOption(row, mapUserToEntity);
