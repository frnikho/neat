import {AnyPgColumn, boolean, jsonb, pgTable, timestamp, varchar} from "drizzle-orm/pg-core";
import {uid} from "@infra/utils/db.utils";
import {user} from "@schema/user.schema";

import * as entity from '@entity/settings.entity';
...
export const mapSettingsOption = <T>(row: Option<Settings>) => mapOption(row, mapSettings);

export const mapSettingsList = (rows: Settings[]) => rows.map(mapSettings<object>)