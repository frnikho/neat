...
import {AllPermissions} from "@service/permission.service";
...
export const defaultRoles: Role[] = [
    {
        name: 'DEFAULT',
        description: '',
        permissions: ['auth.me', 'user.read.own', 'user.write.own', 'user.delete.own', 'role.read.own'],
        default: true,
    },
    {
        name: 'ADMIN',
...
export const checkPermission = (db: NodePgDatabase, perms: string[]) => {
    return permissionRepo(db).findByIds(perms)
        .andThen((permissions) => {
            ...
            if (missing.length > 0) {
                return err(new DbException('Some permissions not found: ' + missing.join(',')));
            }
