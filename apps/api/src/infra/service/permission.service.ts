...
export type PermissionKeysFromArray<T extends readonly Permission[]> = {
  [K in keyof T]: T[K] extends Permission
    ? `${T[K]['resource']}.${T[K]['action']}`
    : never;
}[number];
...
export const hasPermission = (
  ...
  permissionToCheck: AllPermissions
) => {
  return roles.some((role) =>
    role.permissions.some(
      (permission) =>
        `${permission.resource}.${permission.action}` === permissionToCheck
    )
  );
};
...
export const hasEveryPermission = (
  roles: RoleWithPermissions[],
  permissionsToCheck: AllPermissions[]
) => {
