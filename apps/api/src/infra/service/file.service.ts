...
export const buildPublicUrl = <K extends SettingsKey>(db: NodePgDatabase, metadata: FileMetadata, key: keyof SettingsValue<K>) => {
    return settingsRepo(db).find('s3').map((settings) => {
