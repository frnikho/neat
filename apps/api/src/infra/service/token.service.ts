...
import {TokenServiceInterface} from "@interface/token.interface";
import {CacheException} from "@infra/exception/cache.exception";

export default (client: RedisClient): TokenServiceInterface => <TokenServiceInterface>({
    ...
    createTokenPair: (sessionId: string, userId: string) => {
        ...
        return ResultAsync.combine([accessToken, refreshToken])
            .andThen(([access, refresh]) => {
                return tokenRepo(client).insert(`refresh:${sessionId}`, refresh)
    ...
    rotateTokens: (sessionId: string, userId: string) => {
        return tokenRepo(client).get(`refresh:${sessionId}`)
            .andThen(currentRefresh => {
                ...
                return tokenRepo(client).delete(`refresh:${sessionId}`)
                    .andThen(() =>
                        ResultAsync.combine([newAccessToken, newRefreshToken])
                    )
                    .andThen(([access, refresh]) => {
                        return tokenRepo(client).insert(`session:${userId}:${sessionId}`, 'active')
                            .andThen(() => tokenRepo(client).insert(`refresh:${sessionId}`, refresh)
    ...
    invalidateSession: (userId: string, sessionId: string) => {
        return tokenRepo(client).delete(`session:${userId}:${sessionId}`)
