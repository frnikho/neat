...
export default () => {
	...
	const mappedRolesResult = permissionsResult.andThen((permissions) => {
		...
		const mappedRoles = defaultRoles.map((role) => ({
			...role,
			permissions: role.permissions.map((p) => permissionMap.get(p)).filter((id): id is string => !!id),
		}));
		...
		const checks = mappedRoles.map((mappedRole) =>
			roleRepo(db)
				...
				.andThen((maybeRole) => {
					...
					const currentKeys = currentPermissions.map((p) => `${p.resource}.${p.action}`);
					const requiredKeys = defaultRoles.find((r) => r.name === mappedRole.name)?.permissions ?? [];

					const missing = requiredKeys.some((perm) => !currentKeys.includes(perm));
					return ok(missing); // role incomplet => à recréer
				})
				.map((shouldCreate) => (shouldCreate ? mappedRole : null)),
		);

		return ResultAsync.combine(checks).map((res) => res.filter((r): r is (typeof mappedRoles)[number] => r !== null));
	});

	return rolesToCreateResult.andThen((rolesToCreate) => {
		if (rolesToCreate.length === 0) return ok();

		const deleteResult = roleRepo(db).deleteManyByName(rolesToCreate.map((r) => r.name));
			...
			.andThen((createdRoles) => {
				const assignResult = ResultAsync.combine(
					createdRoles.map((createdRole) => {
						const original = rolesToCreate.find((r) => r.name === createdRole.name)!;
						return roleRepo(db).addPermissions(createdRole.id, original.permissions);
					}),
				);
