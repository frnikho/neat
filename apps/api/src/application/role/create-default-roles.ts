...
export default () => {
    const permissionsResult = permissionRepo(db).list(1, 20000);
        ...
        const checks = mappedRoles.map((mappedRole) =>
            roleRepo(db)
                ...
                .andThen((maybeRole) => {
                    ...
                    const requiredKeys = defaultRoles.find(
                        (r) => r.name === mappedRole.name
                    )?.permissions ?? [];
        ...
        return deleteResult.andThen(() => {
            return ResultAsync.combine(
                rolesToCreate.map((role) =>
                    roleRepo(db)
                        .create({ ...role, isBuiltIn: true, isDefault: role.default })
                )
            );
