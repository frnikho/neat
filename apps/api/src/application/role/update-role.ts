...
export default ({ roleId, body }: Input): ResultAsync<Output, Error> => {
    ...
    return repo
        .findById(roleId)
        .andThen((role) =>
            optionToResult(role, appException(apiErrorCodeToStatus.BAD_REQUEST, 'Role not found !'))
        )
        .andThen((role) =>
            repo.update(role.id, body).andThen((updatedRole) => {
                ...
                return opTx(db, (tx) => {
                    ...
                    return checkPermission(tx, body.permissions!)
                        .andThen((perms) =>
                            txRepo
                                .removePermissions(updatedRole.id, perms.map((p) => p.id))
                                .map(() => perms)
                        )
                        .andThen((perms) =>
                            txRepo.addPermissions(updatedRole.id, perms.map((p) => p.id))
                        )
                        .map(() => updatedRole);
                });
            })
        ...
        .andThen((maybeRole) =>
            optionToResult(maybeRole, appException(apiErrorCodeToStatus.BAD_REQUEST, 'Role not found !'))
        )
