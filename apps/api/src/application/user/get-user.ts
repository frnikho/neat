...
export default ({userId, auth}: Input, repo = userRepo(db)): ResultAsync<Output, Error> => {

    if (!canGetUser(auth, userId)) {
        return errAsync(appException(apiErrorCodeToStatus.FORBIDDEN, "You don't have permission to get users"));
    }

    return repo.findUserByIdWithProfilePicture(userId)
        .andThen((user) =>
            optionToResult(user, appException(apiErrorCodeToStatus.NOT_FOUND, `User with id ${userId} not found`))
        )
        .andThen(([user, profilePicture]) => {
            if (isNone(profilePicture)) {
                return okAsync({user, url: undefined});
            }
            return buildPublicUrl(db, profilePicture.value, "user").map((url) => ({user, url}));
        })
        .map(({user, url}): Output => ({
            user,
            profilePicture: url
