    ...
    return userRepo(db).findUserById(userId)
        ...
        .andThen((user) => {
            ...
            if (!user.profilePictureFile) {
                return errAsync(appException(400, 'User does not have a profile picture'));
            }
            const pictureFile = user.profilePictureFile;
            return fileMetadataRepo(db).findById(pictureFile)
                .andThen((f) => optionToResult(f, appException(apiErrorCodeToStatus['BAD_REQUEST'], 'Profile picture file not found')))
