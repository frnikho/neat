...
export default (input: Input): ResultAsync<Output, Error> => {
	...
	const user = userRepo(db)
		...
		.andThen((e) => optionToResult(e, appException(apiErrorCodeToStatus.BAD_REQUEST, "User not found")))
		.andThen((user) => {
			if (!user.password) {
				return err(appException(apiErrorCodeToStatus.BAD_REQUEST, "User has no password set"));
			}
			return ok(user);
		})
		.andThen((user) => {
			return verifyPassword({
				...
				if (!valid) {
					return err(appException(apiErrorCodeToStatus.BAD_REQUEST, "Bad credentials"));
				}
				return ok(user);
			});
		});
