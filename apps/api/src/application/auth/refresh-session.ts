...
export default (
...
): ResultAsync<RefreshTokenOutput, Error> => {
  if (!(input.accessToken && input.refreshToken)) {
    return errAsync(
      appException(apiErrorCodeToStatus.BAD_REQUEST, 'Missing tokens')
    );
  }
  ...
  const access = verifyToken(input.accessToken, true)
    .andThen(({ payload }) =>
      tokenRepo(client).get(`refresh:${payload.sessionId}`)
    )
    .andThen((refresh) =>
      optionToResult(
        refresh,
  ...
  return ResultAsync.combine([access, refresh]).andThen(
    ([accessToken, refreshToken]) => {
      if (accessToken !== input.refreshToken) {
        return err(
          appException(
            apiErrorCodeToStatus.BAD_REQUEST,
            'Invalid refresh token, you need to login again'
          )
        );
      }
      return tokenService(client).rotateTokens(
