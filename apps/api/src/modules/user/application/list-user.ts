import {UserInterface} from "$user/domain/interface/user.interface";
import userRepo from "$user/infra/repo/user.repo";
import {db} from "@core/db";
import {_trace} from "@core/instrumentation";
import {AuthContext} from "$auth/api/auth.middleware";
import {Pagination} from "@core/request";
import {User} from "$user/domain/entity/user.entity";
import {handle} from "@core/type";
import {ResultAsync} from "neverthrow";
import {ApiError} from "@core/exceptions";

type Input = {
    auth: AuthContext;
    pag: Pagination;
}

type Output = User[];

const listUser = ({pag, auth}: Input, repo: UserInterface = userRepo(db)): ResultAsync<Output, ApiError> => {
    const result = repo.list(pag.page, pag.limit);
    return handle(result);
}

export default _trace(listUser, 'app.user/list');